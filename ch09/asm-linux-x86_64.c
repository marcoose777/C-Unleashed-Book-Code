...
INSTRUCTION* FindInstruction (char* Token)
{
   ...
   {
	  int res = strncmp (InstructionTable[i].InstructionName, Token, 6);
	  if (0 == res)
      {
...
void ReadAssembly (unsigned char* Memory, int MemLocation, char* FileName)
{
   FILE*    Fp             = NULL;
   ...
   while (IsOk && NULL != fgets (Line, MAX_LINE, Fp ))
   {
      ...
      else
      {
         ...
         printf("FindInstruction (Token) Token := %s\n",Token);
   ...
   if (NULL != Fp)
      fclose (Fp);
   if(IsOk){
	   printf("Programme file loaded and parsed succesfully!\n\n");
   }
   return;
}
...
int main ( int argc, char **argv)
{
   ...
   unsigned char        Memory[ADDRESS_SPACE] = {0};
